...
void modules::misc::no_visor::callback()
{
	...
	const auto list = driver.read< std::uintptr_t >(camera_copy.instance, { 0x30, 0x30 }); std::uintptr_t visor = 0u;
	...
	for (auto i = 0u; i < size * 0x10; i += 0x10)
	{
		...
		{
			const auto field = driver.read < std::uintptr_t >(index + 0x28);
	...
	if (visor && std::get< bool >(menu::config::config_map["enable_remove_visor"]))
		driver.write< float >(visor + 0xB8, static_cast<float>(std::get< int >(menu::config::config_map["visor_intensity"])));
}