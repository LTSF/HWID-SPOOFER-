...
bool renderer::imgui::elements::functions::config_button(const char* label, const char* data, const char* author)
{
    ...
    if (it_circle->second.size > 65.f)
    {
        ...
        window->DrawList->AddText(bb.Min + ImVec2(130 + 20 - ImGui::CalcTextSize(ICON_FA_TIMES).x / 2, 60 + (120 - 75) / 2 - ImGui::CalcTextSize(ICON_FA_TIMES).y / 2), ImColor(203, 44, 55), ICON_FA_TIMES);
